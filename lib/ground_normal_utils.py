...
def foot2d_to_foot3d(foot2d_in_scene, camM, ground):
    ...
    ground   (4,)
    '''
    foot2d_in_scene = np.concatenate(
        [foot2d_in_scene, np.ones((foot2d_in_scene.shape[0], 1))], axis=-1)  # m, 3
...
def get_xx_yy_v2(h, w, delta_h=0, delta_w=0):  # 优雅 但是并没有快速
...
def visual_GN_patch(image, image_trans, interval=1, save_path=None):
    ...
    for h_i in range(0, h, interval):
        for w_i in range(0, w, interval):
            pt1 = (w_i, h_i)
            pt2_w, pt2_h = int(
                w_i+image_trans[h_i, w_i, 0]), int(h_i+image_trans[h_i, w_i, 1])
            # if pt2_w < 0 or pt2_w >=w or pt2_h <0 or pt2_h>=h:
            #     continue
            pt2 = (pt2_w, pt2_h)
            cv2.arrowedLine(image, pt1, pt2, color=color,
                            thickness=2, tipLength=0.2)
    if save_path is not None:
        cv2.imwrite(save_path, image)


def get_imageGn_by_T(camMats, ground, height, width, delta_h=0, delta_w=0, norm=True):
