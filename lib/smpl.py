...
sys.path.append(os.path.abspath(__file__).replace('src/smpl.py',''))
...
class SMPL(nn.Module):
    ...
    def __init__(self, model_path, J_reg_extra9_path=None, J_reg_h36m17_path=None,\
        ...
        self.gender = gender
        self.age = age
        self.align_by_image_pelvis = True
        if 'align_by_image_pelvis' in kwargs:
            self.align_by_image_pelvis=kwargs['align_by_image_pelvis']
        if not self.align_by_image_pelvis:
            print('self.align_by_image_pelvis', self.align_by_image_pelvis)
        ...
        v_template_numpy=to_np(data_struct.v_template)
        # change the height ofv_template
        change_flag=False
        if change_flag:
            h = v_template_numpy[:, 1].max() - v_template_numpy[:, 1].min()
            v_template_numpy = v_template_numpy / h * 1.5
            new_h = v_template_numpy[:, 1].max() - v_template_numpy[:, 1].min()

        self.register_buffer('v_template',
                             to_tensor(v_template_numpy,
                                       dtype=dtype))
        if betas is None:
