...
def flip_foot2d(foot2d, width=None):
    foot2d = np.array(foot2d).copy()
    if width is not None:
        foot2d[0] = width - foot2d[0]
    else:
        foot2d[0] = - foot2d[0]
    return foot2d
...
def image_crop_pad(image, kp2ds=None, full_bboxs=None, foot2ds=None, ankle2ds=None, crop_trbl=(0, 0, 0, 0), bbox=None, pad_ratio=1.,
                   pad_trbl=None,
                   draw_kp_on_image=False, real_cps=None):
    '''
    ...
    if pad_trbl[0]==pad_trbl[1]==pad_trbl[2]==pad_trbl[3]==0:
        return image, kp2ds, [*image.shape[:2], *crop_trbl, 0,0,0,0], full_bboxs, foot2ds, ankle2ds, real_cps
    ...
    if kp2ds is not None:
        ...
        if real_cps is None:
            ...
            real_cps_aug = [cp - leftTop  for cp in real_cps]

        full_bboxs_aug = None
        if full_bboxs is not None:
            full_bboxs_aug = []
            for bbox_i in full_bboxs:
                new_bbox_i = np.array(bbox_i).copy()
                new_bbox_i[0] = new_bbox_i[0] - leftTop[0, 0]
                new_bbox_i[1] = new_bbox_i[1] - leftTop[0, 1]
                new_bbox_i[2] = new_bbox_i[2] - leftTop[0, 0]
                new_bbox_i[3] = new_bbox_i[3] - leftTop[0, 1]
                full_bboxs_aug.append(new_bbox_i)
        foot2ds_aug = None
        if foot2ds is not None:
            foot2ds_aug = []
            for foot2d_i in foot2ds:
                new_foot2d_i = np.array(foot2d_i).copy()
                new_foot2d_i[0] = new_foot2d_i[0] - leftTop[0, 0]
                new_foot2d_i[1] = new_foot2d_i[1] - leftTop[0, 1]
                foot2ds_aug.append(new_foot2d_i)
        ankle2ds_aug = None
        if ankle2ds is not None:
            ankle2ds_aug = [kp2d - leftTop if kp2d.shape[-1] == 2 else kp2d - leftTop3 for kp2d in ankle2ds]
        return image_aug, kp2ds_aug, [*image_aug.shape[:2], *crop_trbl, *pad_trbl], full_bboxs_aug, foot2ds_aug, ankle2ds_aug, real_cps_aug
    else:
        return image_aug, None, [*image_aug.shape[:2], *crop_trbl, *pad_trbl], None, None
    # else:
    #     return image, kp2ds, np.array([*image.shape[:2], *crop_trbl, 0,0,0,0]), full_bboxs, foot2ds
...
def image_pad_white_bg(image, pad_trbl=None, pad_ratio=1., pad_cval=255):
    ...
    if pad_trbl[0]==pad_trbl[1]==pad_trbl[2]==pad_trbl[3]==0:
        return image, [*image.shape[:2], *[0, 0, 0, 0], *pad_trbl]
    ...
    return image_aug, [*image_aug.shape[:2], *[0, 0, 0, 0], *pad_trbl]



def flip_ankle2ds(kps, width=None):
    if width is not None:
        kps[:, 0] = width - kps[:, 0]
    else:
        kps[:, 0] = - kps[:, 0]
...
def process_image(originImage, full_kp2ds=None, full_bboxs=None, foot2ds=None, ankle2ds=None, augments=None, is_pose2d=True,
                  multiperson=False, data_scale=1, real_cps=None, scene_shape=None):
    ...
    image_aug, kp2ds_aug, offsets, bbox_aug, foot2d_aug, ankle2ds_aug, real_cps_aug = image_crop_pad(originImage, kp2ds=full_kp2ds, full_bboxs=full_bboxs,
                                                             foot2ds=foot2ds, ankle2ds=ankle2ds, crop_trbl=crop_trbl, bbox=bbox,
                                                             pad_ratio=1., real_cps=real_cps)

    offsets.append(data_scale)
    offsets.append(scene_shape[0]) # scene_w
    offsets.append(scene_shape[1]) # scene_h
    offsets=np.array(offsets) 
    return image_aug, orgImage_white_bg, kp2ds_aug, offsets, bbox_aug, foot2d_aug, ankle2ds_aug, real_cps_aug
