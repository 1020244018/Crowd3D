...
class SMPLR(nn.Module):
    def __init__(self, use_gender=False, transl=False):
        ...
        if use_gender:
            ...
            self.smpl_neutral = SMPL(model_path, J_reg_extra9_path=args().smpl_J_reg_extra_path, J_reg_h36m17_path=args().smpl_J_reg_h37m_path, gender='neutral',create_transl=transl)
            self.smpls = {'n':self.smpl_neutral}

    def forward(self, pose, betas, gender='n', transl=None):
        ...
        if transl is not None:
            outputs = self.smpls[gender](poses=pose, betas=betas, transl=transl)
        else:
