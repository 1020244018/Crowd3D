...
class Image_processor(Predictor):
    ...
    @torch.no_grad()
    def run(self, image_root, ground_cam_path, test_dataset, have_people_image_json):
        os.makedirs(self.output_dir, exist_ok=True)
        self.visualizer.result_img_dir = os.path.join(self.output_dir, 'images_results')
        counter = Time_counter(thresh=1)

        internet_loader = self._create_single_data_loader(dataset=test_dataset, train_flag=False, image_root=image_root, ground_cam_path=ground_cam_path, have_people_image_json=have_people_image_json, shuffle=False)
        counter.start()
        ...
        for test_iter, meta_data in enumerate(internet_loader):
            ...
            if self.save_visualization_on_img:
                show_items_list = ['mesh','org_img', 'pj2d',  'hvip2d'] #['pj2d_overlap',]
...
def main():
    ...
    with ConfigContext(parse_args(input_args)):
        ...
        if hasattr(args(), 'ground_cam_path'):
            ground_cam_path=args().ground_cam_path
        else:
            print('lack ground_cam_root')
            exit()
        
        if hasattr(args(), 'have_people_image_json'):
            have_people_image_json=args().have_people_image_json
        else:
            have_people_image_json=''
       
        processor.run(inputs, ground_cam_path, args().test_dataset, have_people_image_json)
